...
- When I say "ready to push" (or similar), execute the full flow interactively: (1) ensure the current revision has an accurate `jj desc`, (2) **CRITICAL: FIRST rebase the current revision onto `main@origin` with `jj rebase -r @ -d main@origin`** - do NOT skip this step or the push will have the wrong parent (testing instead of remote main), (3) verify the rebase worked with `jj log -r 'ancestors(@, 3)'` to confirm @ is now based on main@origin, (4) move the `main` bookmark to the rebased current revision with `jj bookmark set main -r @`, (5) push with `jj git push -r main`, (6) rebase `testing` onto the new main with `jj rebase -b testing -d main`, (7) push testing with `jj git push -b testing`, (8) rebase other bookmarks onto the pushed `main`, (9) spawn new `latest updates` from `testing` with `jj new testing -m "latest updates"`. Halt and ask for guidance if any step hits conflicts.
